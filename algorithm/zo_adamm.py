...
class ZoAdaMM:
    def __init__(self, a=0.1, c=0.5, A=100, alpha=0.602, gamma=0.101,
                 iter_num=5000, rep_num=10, direct_num=6,
    ...
    def get_grad_est(self, theta, iter_idx=0, rep_idx=0): 
        ...
        for direct_idx in range(self.direct_num):
            ...
            direct = delta / np.linalg.norm(delta)

            eval_img_idx = np.random.choice(self.n, self.eval_n, replace = False)
            loss_base = self.loss_noisy(eval_img_idx, theta)
            eval_img_idx = np.random.choice(self.n, self.eval_n, replace = False)
            loss_plus = self.loss_noisy(eval_img_idx, theta + c_k * direct)
            grad_all[:,direct_idx] = (loss_plus - loss_base) / c_k * direct * self.p
    ...
    def train(self, img_vec):
        ...
        for rep_idx in range(self.rep_num):
            ...
            # reset estimate
            theta = self.theta_0
            self.v = 1e-6 * np.ones(self.p)
            self.v_hat = 1e-6 * np.ones(self.p)
